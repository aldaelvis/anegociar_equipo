...
class GestorCategoriasController{
	...
	public function MostrarCategoriasYSubcategoriasController(){
		...
		foreach($respuesta as $row) {
   		...
   		foreach($categories as $cat) {
		    echo '<div class="anegociar-categorias-subcategorias">
		    		<div class="categorias-subcategorias">'.'
		    			<a href="'.$cat['name'].'.html">
		    				<img src="'.$ruta_imagenes.$cat['img'] .'" alt="anwegociarperu-imagen-categorias" class="img-responsive">'.'
		    			</a>
		    			<h5>
		    				<a href="'.str_replace( "%20", " ", $cat['name']).'.html">'.$cat['name'].'
		    				</a>
		    			</h5>
		    		<ul>';
		    foreach($cat['products'] as $prod) {
		        echo '<li>
		        		<a href="'. $cat['name'] .'_' . $prod['name'] . '.html">' . $prod['name'] . '
		    			</a>
		        	</li>';
	...
	public function mostrarClasificadosPorCategoriaVehiculosController(){
		
		$respuesta = GestorCategoriasModel::mostrarClasificadosPorCategoriaVehiculosModel("Tclasificados"); 
							...
							<div class="campo-imagen">
								<div class="imagen">
									<a href="ver_clasificado-'.$item["idclasificado"].'-'.str_replace( " ", "_", $item["titulo"]).'.html">';
										if($item["nombreimagen"] == '' || $item["nombreimagen"] == null ){
											echo '<img src="'.$ruta.'logo.png" alt="No hay imagen" class="img-thumbnail">';
										} else {
											echo '<img src="'.$ruta.$item["nombreimagen"].'" class="img-thumbnail">';
										}
	...
	public function mostrarClasificadosPorCategoriaInmueblesController(){
		
		$respuesta = GestorCategoriasModel::mostrarClasificadosPorCategoriaInmueblesModel("Tclasificados"); 
...
*/
		...
		$consulta_total_clasificados = GestorCategoriasModel::contarClasificadosModel($captura_Categoria, "Tclasificados");
		...
		*/
		...
		$total_clasificados = $consulta_total_clasificados["contar"];
		#echo 'contar'.$total_clasificados;
...
$new_date = date('d-m-Y', $timestamp);
				// <a href="index.php?action=ver_clasificado&id='.$item["idclasificado"].'&titulo='.$item["titulo"].'">
				echo'
					<div class="bloque-lista-clasificados">
			 			<div class="lista-clasificados-campos">
							<div class="campo-imagen">
	...
	public function mostrarClasificadosPorSubCategoriaController(){
	/*
	$item_limit = 10; // 10 Results a Page
	$page_set = isset($_GET{'page_number'});
	$page = $page_set ? $_GET{'page_number'}+1:0;
	...
	echo '<h1>'.$offset.'/<h1>';
		...
		$captura_SubCategoria = $_GET["subcat"];
		#echo $captura_SubCategoria;
		...
		*/
		...
		$respuesta = GestorCategoriasModel::mostrarClasificadosPorSubCategoriaModel($captura_Categoria, $captura_SubCategoria, $mostrar_desde, $cantidad_elementos, "Tclasificados"); 
		...
		if (!empty($respuesta)){
				...
				echo'
					<div class="bloque-lista-clasificados">
			 			<div class="lista-clasificados-campos">
							...
							<div class="campo-codigo-revista">
								<a href="index.php?action=ver_clasificado&id='.$item["idclasificado"].'">
									<i class="far fa-newspaper"></i>&nbsp; Cod. Revista: <b>BNN4NB</b>
								</a>
							</div>
							<div class="campo-tipo-moneda">
								<a href="index.php?action=ver_clasificado&id='.$item["idclasificado"].'">
			...
			#echo $row;
	 		...
	 		for ($i=1;$i<=$numero_de_paginas;$i++) {
		  		echo '
		  			<div class="pagina">
		  				<a href="index.php?action=subcategorias_clasificados&cat='.$captura_Categoria.'&subcat='.$captura_SubCategoria.'&page=' . $i . '">' . $i . '</a>
					</div>
		  			';
		  		}
