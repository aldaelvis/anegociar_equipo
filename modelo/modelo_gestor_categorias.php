...
class GestorCategoriasModel{
	...
	public function verificarIdModeloModel($datosModel, $tabla){
		...
		$stmt = Conexion::conectar()->prepare("SELECT idmodelo, nombre_modelo FROM $tabla WHERE nombre_modelo = :mod");

		$stmt->bindParam(":mod", $datosModel, PDO::PARAM_STR);	
		$stmt->execute();
		return $stmt->fetch();
		$stmt->close();
	}
...
*/
		...
		#------------------------------------------------------
	public function contarClasificadosPorSubcategoriaModel($datosModel, $datosModel1, $tabla){

		$stmt = Conexion::conectar()->prepare("SELECT COUNT(idclasificado) as contar FROM $tabla INNER JOIN Tdetalles_caracteristicas_clasificados ON Tclasificados. iddetalle_caracteristica_clasificado = Tdetalles_caracteristicas_clasificados. iddetalle_caracteristica_clasificado INNER JOIN Tcategorias ON Tdetalles_caracteristicas_clasificados. idcategoria = Tcategorias. idcategoria INNER JOIN Tsubcategorias ON Tdetalles_caracteristicas_clasificados. idsubcategoria = Tsubcategorias. idsubcategoria WHERE Tclasificados. estado = '1' AND nombre_categoria = :cat AND nombre_subcategoria = :subcat");
